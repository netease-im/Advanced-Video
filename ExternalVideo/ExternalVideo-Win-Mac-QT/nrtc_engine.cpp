...
public:
    NRTCEngineEvent(NRTCEngine& engine) : m_engine(engine) {}
    ...
    virtual void onUserAudioStop(nertc::uid_t uid) override { m_engine.stopRemoteAudio(uid); }

    virtual void onUserAudioStart(nertc::uid_t uid) override { m_engine.startRemoteAudio(uid); }
...
NRTCEngine::NRTCEngine(QObject* parent) : QObject(parent), rtc_engine_handler_(new NRTCEngineEvent(*this)) {
    rtc_engine_ = nullptr;
    ...
    // set the path of log before init the NERTCEngine
...
int NRTCEngine::joinChannel(const QString& token,
...
bool NRTCEngine::setExternalVideoSource(bool open) {
    if (open) {
        rtc_engine_->enableLocalVideo(false);
        video_device_manager->setDevice(kNERtcExternalVideoDeviceID);  //设置为外部设备采集
        return rtc_engine_->setExternalVideoSource(open);
    }
    return true;
}

int NRTCEngine::pushExternalVideoFrame(NERtcVideoFrame* frame) {
    return rtc_engine_->pushExternalVideoFrame(frame);
}
