...
typedef struct {
    ...
    st_mobile_animal_result_t* p_animal;        ///< 动物脸检测结果
...
typedef struct st_gan_request_t {
    char category[EFFECT_MAX_NAME_LEN];    ///< GAN类型名称（大类别）。目前支持AttrEdit,GanSkin,GanHair,GanAllStyle 和部署的服务器支持功能相关
    char function[EFFECT_MAX_NAME_LEN];                   ///< 服务器上对应的function, 表示GAN类型名称(小类别)
