...
typedef struct st_mobile_human_action_segments_t {
    ...
    int skin_count;                          ///  目前支持多人皮肤分割，分割face_id与p_faces的id对应
...
typedef enum {
    ...
    /// 设置106点、240点是否平滑(默认值0 不平滑),不平滑点会有些抖动，平滑点会有点滞后.
    ...
    /// 设置人头实例分割，人头实例分割依赖人脸检测，且只有加载人头实例分割模型设置该参数有效．默认不开启，出单个mask; 开启则多人多mask，数量跟人脸个数对应．
...
// 皮肤分割参数
    ...
    ///设置皮肤实例分割，皮肤实例分割依赖人脸检测，且只有加载皮肤实例模型设置该参数有效．默认不开启，只单个mask; 开启出多人多mask，数量跟人脸个数对应．
...
// 面部遮挡分割参数
    ...
    ///设置人脸遮挡实例分割，人脸遮挡实例分割依赖人脸检测，默认不开启，出单个mask; 开启则多人多mask，数量跟人脸个数对应．
