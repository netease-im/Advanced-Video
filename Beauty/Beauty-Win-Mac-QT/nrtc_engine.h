...
public:
    ...
    bool isSmoothEnable();
    void renderFrame(int ntype, void *data, uint32_t width, uint32_t height,int frame_id);
...
signals:
    ...
    void renderFrameSig(int ntype, void *data, uint32_t width, uint32_t height,int frame_id);
private:
    // 网易云通信 SDK 引擎
