...
void NRTCEngineEvent::onUserVideoStop(nertc::uid_t uid) {
    emit m_engine.videoStopped(uid);
    m_engine.stopRemoteVideo(uid);
}

void NRTCEngineEvent::onUserVideoStart(nertc::uid_t uid, NERtcVideoProfileType max_profile) {
    emit m_engine.videoStart(uid, (int) max_profile);
    m_engine.startRemoteVideo(uid);
}

void NRTCEngineEvent::onUserAudioStop(nertc::uid_t uid)  {
    m_engine.stopRemoteAudio(uid);
}

void NRTCEngineEvent::onUserAudioStart(nertc::uid_t uid)  {
    m_engine.startRemoteAudio(uid);
}

void NRTCEngineEvent::onJoinChannel(channel_id_t cid, nertc::uid_t uid, NERtcErrorCode result, uint64_t elapsed) {
    if (!result){
...
void NRTCEngineEvent::onUserJoined(nertc::uid_t uid, const char * user_name) {
    qDebug("[INFO] Remote user %lld joined channel successuflly.", uid);
    emit m_engine.userJoined((quint64)uid);
}

void NRTCEngineEvent::onUserLeft(nertc::uid_t uid, NERtcSessionLeaveReason reason) {
    qDebug("[INFO] Remote user %lld left channel.", uid);
    emit m_engine.userLeft((quint64)uid);
}

void NRTCEngineEvent::onRtcStats(const NERtcStats &stats)  {
    emit m_engine.rtcStats(stats);
}
