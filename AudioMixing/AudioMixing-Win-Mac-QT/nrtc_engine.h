...
#define APP_KEY "c96938419c4d4f0ffe8f169c2c43d638"          // put your app key here, testing
...
public:
    ...
    int startAudioMixing(nertc::NERtcCreateAudioMixingOption *option);
    ...
    int playEffect(uint32_t effect_id, nertc::NERtcCreateAudioEffectOption *option);
    int stopEffect(uint32_t effect_id);

    int setEffectSendVolume(uint32_t effect_id, uint32_t volume);

    int setEffectPlaybackVolume(uint32_t effect_id, uint32_t volume);
signals:
    void joiningChannel();
    void leavingChannel();
